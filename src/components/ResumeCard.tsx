...
const resumeStyles = tv({
  base: 'mb-4 flex-row items-center justify-between rounded-md border-l-4 bg-white py-2 px-3 shadow-sm dark:bg-glassDark w-full h-16',
  variants: {
    cardType: {
...
export function ResumeCard({ type, value, isOutcome = true }: ResumeCardProps) {
  ...
  return (
    <View className={resumeStyles({ cardType: type })}>
      <Text className="text-lg font-bold text-black dark:text-white">{type}</Text>
